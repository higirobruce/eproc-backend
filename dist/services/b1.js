...
function getVatGroups() {
    return (0, sapB1Connection_1.sapLogin)()
        ...
        return fetch(`${process.env.IRMB_B1_SERVER}:${process.env.IRMB_B1_SERVICE_LAYER_PORT}/b1s/v1/VatGroups?$filter=Inactive eq 'tNO'&$select=Code,Name,Category`, {
            method: "GET",
            headers: {
...
function getFixedAssets() {
    return (0, sapB1Connection_1.sapLogin)()
        ...
        let COOKIE = res.headers.get("set-cookie");
        localstorage.setItem("cookie", `${COOKIE}`);
        return fetch(`${process.env.IRMB_B1_SERVER}:${process.env.IRMB_B1_SERVICE_LAYER_PORT}/b1s/v1/Items?$filter= ItemType eq 'itFixedAssets' and CapitalizationDate eq null &$select=ItemCode,ItemName`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                Cookie: `${localstorage.getItem("cookie")}`,
...
function getBusinessPartnerByName(CardName) {
    return (0, sapB1Connection_1.sapLogin)()
        ...
        let COOKIE = res.headers.get("set-cookie");
        localstorage.setItem("cookie", `${COOKIE}`);
        return fetch(`${process.env.IRMB_B1_SERVER}:${process.env.IRMB_B1_SERVICE_LAYER_PORT}/b1s/v1/BusinessPartners?$select=CardName,CardCode&$filter=CardName eq '${CardName}'`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                Cookie: `${localstorage.getItem("cookie")}`,
